...
with tab3:
    ...
    descriptions = {
        '垂直跳び': '下肢全体の純粋な爆発的パワー。反動、腕ふりを最大限活用して、全身の連動も評価している。スプリントのスタートやフィールド競技の寄与率が高い。簡易的に測定が可能なため、定期的にとることをおすすめします',
        'DJ_RSI': '下肢のバネ性能を示す反応筋力指数。もしも1秒間、地面に力を加え続けることができるなら、何メートル跳ぶことができるかを表した指標。スプリントの最高速度の局面での寄与率が高い。',
        '立ち幅跳び': '水平方向への爆発的パワー発揮能力。反動、腕ふりを最大限活用して、全身の連動も評価しつつ、垂直ジャンプと異なり、「水平に力を発揮する技術」を見ることができる。',
        '12段跳び': '連続跳躍による推進力と弾性エネルギーの再利用。よりスプリントに近く、「バウンディングの上手さ」という点での評価も可能。',
        '幅跳び/下肢長': '下肢の長さを考慮した水平方向への相対的なパワー。足が長ければ長い程力積を稼ぐ、時間的余裕とリーチがあるため、その影響を排除した評価基準。', 
        '前投げ': '体幹から上半身への力の伝達。全身パワーを評価し、重心や他の物に力を加える身体操作の技術が要されるためテクニカルな視点で見る必要もある。',
        '後ろ投げ': '股関節伸展、腰背部パワーを評価。重心や他の物に力を加える身体操作の技術が要されるためテクニカルな視点で見る必要もある。',
        'SQ_1RM': 'すべてのパワーの土台となる基礎筋力のベーシックな評価。',
        '懸垂': '上半身の引く筋力および筋持久力。米軍式の基準で男子は20回、女子は10回で満点。部では苦手な傾向にあるため、陸上競技への直接の寄与は低い種目が多いが、能力としてできるようになろう。',
        'スクワット/体重': '体重に対する相対的な基礎筋力。男子は2.0、女子は1.6程の値まではベーシックで持っていてほしい。', 
        'RAST_max_bw': '無酸素運動における最高出力',
        'RAST_min_bw': '疲労状態での底力',
        'RAST_mean_bw': '無酸素運動を持続するための総合容量',
        '減少率/SEC': 'パワー減少率（低いほど疲労耐性が高い）', 
        'シャトルラン': '有酸素性能力（全身持久力）。冬季練習中のカーディオなどでの活用もできるため、全力で、追いつけなくなるまで取り組むように。'
    }
